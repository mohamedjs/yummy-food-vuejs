            ...
            <div class="breadcrumb-group">
              ...
              <div class="breadcrumb clearfix">
                ...
                <span itemscope="" itemtype="#">
                  <a :href="url" title="cart" itemprop="url"><span itemprop="title">your Cart</span></a>
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="row">
          <div id="shopify-section-cart-template" class="shopify-section">
            <div class="cart-inner">
              <div id="cart">
                <div class="cart-form">
                  <form :action="url+1" method="post" id="cartform">
                    @csrf
                    @method('patch')
                      ...
                      <tbody>
                        <tr v-for="product in products">
                          <td class="image">
                            <div class="product_image">
                              <a :href="base_uri+'/product/'+product.product_id">
                                <vue-load-image>
                                   <img slot="image" :src="url_image+'admin/product/'+product.image" :alt="product.product_name"/>
                          ...
                          <td class="qty">
                            <div class="quantity-wrapper">
                              <div class="wrapper">
                                <input type="text" size="4" :value="product.quantity" @input="change_quan" @change="change_quantity(product._id)"  class="tc item-quantity quantity-value">
                              </div>
                              <!--End wrapper-->
                            </div>
                          ...
                          <td class="remove"><a href="#" @click.prevent="remove_cart(product._id)" class="cart"><span class="lnr lnr-trash"></span></a></td>
                        </tr>
                        <tr class="summary">
                    ...
                    <div class="group-button">
                      <a :href="base_uri" class="_btn">Continue Shopping</a>
                      <div class="group_btn_right">
                        <input type="button" class="_btn clearAllCart" @click.prevent="remove_all" value="Clear all">
                        ...
                        <div id="shipping-calculator">
                          ...
                          <div class="group_form">
                            <div class="form-item">
                              ...
                              <select id="address_country" name="address[country]" v-model="check_out.city" data-default="United States"><option value="---" data-provinces="[]">---</option>
                      ...
                      <div class="checkout-buttons col-sm-6 col-xs-12">
                        <div class="note-content group_form">
                          <div class="form-item">
                            ...
                            <textarea id="note" name="note" v-model="check_out.info" placeholder="Enter text here..."></textarea>
                          </div>
                          <input type="hidden" name="shipping_val" v-model="check_out.ship_val">
                        </div>
                        <div class="checkout-content">
                          <div class="buttons clearfix">
                            <input type="submit" id="checkout" class="_btn" @click.prevent="checkout" name="checkout" value="Check Out">
                          </div>
                        </div>
                      </div>
                    </div>
...
<script type="text/javascript">
  const new_vu = new Vue({
    el:"#app_cart",
    data:{
      products:[],
      check_out:{'info': '' , 'ship_val':'' , 'state' : '' , 'zip' : '' , 'city' : ''},
      sub_total:'',
      quantity:0,
      loading:false,
      url:'{{asset(App::getLocale()."/cart")}}/',
      url_image: '{{asset("/")}}',
    ...
    methods:{
      ...
      change_quantity(id){
        ...
        if(this.quantity > 0){
        this.loading = true;
        axios.post(this.url+'change/quantity/price/'+id+'/'+this.quantity).then(function(response){
          _this.products = response.data.products
          _this.sub_total= response.data.sub_total
          _this.loading = false;
        })
      ...
      remove_all(){
        var _this = this;
        axios.delete(this.url+'remove/all').then(function(response){
          _this.products = response.data.products
          _this.sub_total= response.data.sub_total
          _this.loading = false;
        })
      ...
      calculate_shipping(){
        var _this = this;
        axios.get(this.url+'calculate/shipping').then(function(response){
          console.log(response);
          _this.check_out.ship_val = response.data;
          $('#shipping_val').html(response.data+'$');
        })
      },
      checkout(){
        var _this = this;
        axios.post(this.url+'checkout/cart',this.check_out).then(function(response){
          window.location.href = "{{asset(App::getLocale().'/profile')}}"
        })
      }
    },
    mounted(){
